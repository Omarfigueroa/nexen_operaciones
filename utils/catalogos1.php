...
$query_empresas= "SELECT *  FROM [EMPRESAS] WHERE [ESTATUS] = 'A' ORDER BY Razon_Social ASC";
...
$query= "SELECT * FROM [dbo].[Catalogo_Bancos] WHERE [ESTATUS] = 'A' ORDER BY [NOMBRE_BANCO] ASC";
$consultar_catalogo_banco=$conn_bd->prepare($query);
$consultar_catalogo_banco -> execute();
$result_catalogo_banco = $consultar_catalogo_banco -> fetchAll(PDO::FETCH_ASSOC);
if( $result_catalogo_banco === false) {
    die( print_r( sqlsrv_errors(), true) );
}

$query= "SELECT * FROM [dbo].[Proveedores_Cuentas] WHERE [ESTATUS] = 'A' ORDER BY [Alias] ASC";
$consultar_catalogo_proveedoresCuentas=$conn_bd->prepare($query);
$consultar_catalogo_proveedoresCuentas -> execute();
$result_catalogo_proveedoresCuentas = $consultar_catalogo_proveedoresCuentas -> fetchAll(PDO::FETCH_ASSOC);
if( $result_catalogo_proveedoresCuentas === false) {
    die( print_r( sqlsrv_errors(), true) );
}
